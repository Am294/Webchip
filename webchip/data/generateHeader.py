...
def subsequent_entries(entry, header, cat_variables, quant_variables):
    ...
    for i in entry:
        ...
        if i not in cat_variables and i not in quant_variables:
            print("ERROR" + i + " is an extra variable in dictonary\n")
    ...
    if len(found_cat_variables) != len(cat_variables):
        ...
        print("ERROR: The following dictonary is missing the following categorical variables " + missing_vars)
        print(json.dumps(entry, indent=2))
...
def add_header(header, file_path, sas_file):
    ...
    try:
        with open(file_path, mode='w') as json_file:
            ...
            print(file_path + " successfully added header")
    except:
        print("ERROR: " + file_path + " failed to add header")
        return False
...
if __name__ == "__main__": 
    if len(sys.argv) != 2:
        print("ERROR: Invalid number of command line arguments. Should have 2 not " + len(sys.argv) )
        exit(0)
